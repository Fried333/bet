...
bits320 field_mul(const bits320 in2,const bits320 in)
{
...
bits256 curve25519(bits256 mysecret,bits256 basepoint)
{
    ...
    return(fcontract(field_mul(x,crecip(z))));
}
...
bits320 field_mul(const bits320 in,const bits320 in2)
{
       ...
       const bits320 qmqp) // input Q - Q'
{
    ...
    *xprime = fsum(*xprime,*zprime), fdifference_backwards(zprime->ulongs,origxprime.ulongs);
    xxprime = field_mul(*xprime,*z), zzprime = field_mul(*x,*zprime);
    origxprime = xxprime;
    xxprime = fsum(xxprime,zzprime), fdifference_backwards(zzprime.ulongs,origxprime.ulongs);
    *x3 = fsquare_times(xxprime,1), *z3 = field_mul(fsquare_times(zzprime,1),qmqp);
...
inline bits320 crecip(const bits320 z)
{
    ...
    /* 9 */ b = field_mul(t0, z); // b = 9
    /* 11 */ a = field_mul(b, a); // a = 11
    /* 22 */ t0 = fsquare_times(a, 1);
    /* 2^5 - 2^0 = 31 */ b = field_mul(t0, b);
    /* 2^10 - 2^5 */ t0 = fsquare_times(b, 5);
    /* 2^10 - 2^0 */ b = field_mul(t0, b);
    /* 2^20 - 2^10 */ t0 = fsquare_times(b, 10);
    /* 2^20 - 2^0 */ c = field_mul(t0, b);
    /* 2^40 - 2^20 */ t0 = fsquare_times(c, 20);
    /* 2^40 - 2^0 */ t0 = field_mul(t0, c);
    /* 2^50 - 2^10 */ t0 = fsquare_times(t0, 10);
    /* 2^50 - 2^0 */ b = field_mul(t0, b);
    /* 2^100 - 2^50 */ t0 = fsquare_times(b, 50);
    /* 2^100 - 2^0 */ c = field_mul(t0, b);
    /* 2^200 - 2^100 */ t0 = fsquare_times(c, 100);
    /* 2^200 - 2^0 */ t0 = field_mul(t0, c);
    /* 2^250 - 2^50 */ t0 = fsquare_times(t0, 50);
    /* 2^250 - 2^0 */ t0 = field_mul(t0, b);
    /* 2^255 - 2^5 */ t0 = fsquare_times(t0, 5);
    /* 2^255 - 21 */ return(field_mul(t0, a));
}
